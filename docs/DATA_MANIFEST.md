...
| Boundaries — NCCN study areas (original analysis, 1985-2009/10/11) | Natasha Antonova's "LPa01" 10-mile-buffer study-area boundaries, 1 polygon per park (LEWI split into N/S units) | `data/raw/boundaries/nps/Prithvi_NCCN/LPa01_LEWI_MORA_NOCA_OLYM.shp` (orig. ZIP `LPa01_LEWI_MORA_NOCA_OLYM.zip` kept alongside) | Shapefile | Received 2026-09-24. Authoritative per `CREATED_BY`/`SOURCE` attributes ("N. Antonova" / "NCCN Landscape Dynamics Monitoring Protocol"). Authoritative AOI for LEWI's currently-used dataset (99.94% area containment, `outputs/qa/nccn_aoi_generation_mapping_qa.csv`); documented as study-area context, not used to clip pixel counts (Task 1 decision 2026-09-24, notebook Appendix A.6) |
| Boundaries — NCCN study areas (later analysis, 1987-2017) | Natasha's "Protected Areas" study areas (NPS lands + USFS Wilderness), MORA/NOCA/OLYM only — no LEWI file in this set | `data/raw/boundaries/nps/Prithvi_NCCN/{MORA,NOCA,OLYM}_USFS_NPS_StudyArea.shp` (orig. ZIP `NCCN_Protected_Areas.zip` kept alongside) | Shapefile (parcel-level, multiple features per park) | Received 2026-09-24. This is the boundary NCCN's own `In_Park` field is computed against (see `docs/data_inventory.md` §2.3/§12.1). Authoritative AOI for MORA/NOCA/OLYM's currently-used V2.1.1 datasets (88.22/98.12/97.81% area containment); documented as study-area context, not used to clip pixel counts (Task 1 decision 2026-09-24, notebook Appendix A.6) |
...
| Boundaries — GLKN | LandTrendr per-park analysis-area AOI, 9 units (the 7 `park_code` values used elsewhere in this project, plus PIRO and GRPO which have no reference disturbance data here) | `data/raw/boundaries/glkn/GLKN_LandTrendr_AOIs.shp` (orig. ZIP `GLKN_LT_AOIs.zip` kept alongside) | Shapefile | Received 2026-09-22. FGDC lineage confirms this is the actual per-park LandTrendr change-detection analysis extent (built up park-by-park 2011-2022, exported 2026-04-22) — i.e. the boundary the GLKN reference disturbance data was originally generated within. Containment QA: `outputs/qa/glkn_landtrendr_aoi_containment_qa.csv` (99.99-100% of existing attributed area/count falls inside, all 7 parks). Documented as study-area context (notebook Appendix B.6); `park_code` remains the subregion identity and pixel counts are not AOI-clipped (Task 1 decision 2026-09-24) |
